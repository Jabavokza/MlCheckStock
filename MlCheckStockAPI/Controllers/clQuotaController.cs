    ...
    public class clQuotaController : ApiController
    {
        ...
        private mlRESCheckStock CNTxBill(mlREQCheckStock poREQCheckStock)
        {
            ...
            try
            {
                ...
                {
                    //int nCnt = 0;
                    //nCnt = int.Parse(oDtCnt.Rows[0]["T03"].ToString()) - int.Parse(oDtCnt.Rows[0]["T05"].ToString());
                    //nCnt = nCnt + int.Parse(oDtCnt.Rows[0]["T04"].ToString());
                    oC_RESQta = new mlRESCheckStock()
                    {
                        // tML_Result = oDtCnt.ToString(), //*kt 61-10-30
